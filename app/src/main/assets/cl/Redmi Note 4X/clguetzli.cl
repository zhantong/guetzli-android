...
void ButteraugliFFTSquared(__generic double block[kBlockSize]) {
...
void CoeffToIDCT(__generic const coeff_t block[8*8], uchar out[8*8])
{
...
// out = [YUVYUV....YUVYUV]
void PixelToYUV(ushort pixels_[8 * 8], __generic uchar out[8 * 8], int xsize/* = 8*/, int ysize/* = 8*/)
{
    const int stride = 3;
...
void CoeffToYUV16x16(__generic const coeff_t block[8 * 8], uchar *yuv, __global const ushort *pixel_orig, int block_x, int block_y, int width_, int height_)
{
    uchar idct[8 * 8];
    CoeffToIDCT(&block[0], &idct[0]);
...
void CoeffToYUV8x8(__generic const coeff_t block[8 * 8], uchar *yuv)
{
    uchar idct[8 * 8];
    CoeffToIDCT(&block[0], &idct[0]);
